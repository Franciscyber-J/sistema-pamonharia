...
async function criarTabelas() {
  ...
  await db.query(`
    CREATE TABLE IF NOT EXISTS combos (
        ...
        quantidade_itens_obrigatoria INTEGER NOT NULL
    )
  `);
  ...
  await db.query(`
    CREATE TABLE IF NOT EXISTS regras_combo (
        ...
        setor_id_alvo INTEGER,
        produto_base_id_alvo INTEGER,
        upcharge REAL DEFAULT 0,
        FOREIGN KEY (combo_id) REFERENCES combos (id) ON DELETE CASCADE,
        FOREIGN KEY (setor_id_alvo) REFERENCES setores (id) ON DELETE CASCADE,
        FOREIGN KEY (produto_base_id_alvo) REFERENCES produtos_base (id) ON DELETE CASCADE
    )
  `);

  console.log("Estrutura de tabelas garantida, incluindo combos.");
}

async function cadastrarDadosIniciais() {
  console.log("Iniciando cadastro de dados iniciais...");

  // Cadastros existentes (setores, produtos, etc.) - sem alterações
  ...
  // O restante dos cadastros continua igual...
  const variacoes = [
  ...
  await db.query(`
      INSERT INTO combos (id, nome, descricao, preco_base, quantidade_itens_obrigatoria, imagem_url) 
      VALUES (
          1, 
          'Combo 10 Pamonhas – Monte do seu jeito!', 
          'Economize levando 10 pamonhas! Escolha 10 unidades do nosso setor de Pamonhas e Derivados e monte o combo perfeito para você.',
          119.90,
          10,
          'https://res.cloudinary.com/dznox4s9b/image/upload/v1718155985/pamonharia/1749601075933-combo-10-pamonhas-%C3%A2%E2%82%AC%E2%80%9C-monte-do-seu-jeito_.webp'
      ) ON CONFLICT (id) DO NOTHING
  `);
  // Regras para o Combo de Pamonhas
  await db.query(`INSERT INTO regras_combo (combo_id, setor_id_alvo) VALUES (1, 1) ON CONFLICT DO NOTHING`); // Regra 1: Pode escolher do setor 1
  await db.query(`INSERT INTO regras_combo (combo_id, produto_base_id_alvo, upcharge) VALUES (1, 2, 2.00) ON CONFLICT DO NOTHING`); // Regra 2: Pamonha à Moda (ID 2) tem acréscimo de R$2.00

  await db.query(`
      INSERT INTO combos (id, nome, descricao, preco_base, quantidade_itens_obrigatoria, imagem_url) 
      VALUES (
          2, 
          'Combo 10 Bolinhos – Leve mais, pague menos!', 
          'Aproveite nosso pacote econômico! Escolha 10 dos seus bolinhos ou salgados favoritos e pague um preço especial.',
          35.99,
          10,
          'https://res.cloudinary.com/dznox4s9b/image/upload/v1718155985/pamonharia/1749600932645-combo-10-bolinhos-%C3%A2%E2%82%AC%E2%80%9D-leve-mais,-pague-menos_.webp'
      ) ON CONFLICT (id) DO NOTHING
  `);
  // Regra para o Combo de Bolinhos
  await db.query(`INSERT INTO regras_combo (combo_id, setor_id_alvo) VALUES (2, 2) ON CONFLICT DO NOTHING`); // Regra 1: Pode escolher do setor 2

  console.log("Combos iniciais cadastrados.");
